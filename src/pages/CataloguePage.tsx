...
import { Plus, Edit, Trash2, Package, Star, Loader2, Tag as TagIcon } from 'lucide-react'
...
import { getCatalogueLabel, getCatalogueItemNoun } from '../utils/businessType'
import {
  Button,
  SearchBar,
  EmptyState,
  ...
  return (
    <div className="space-y-6 pb-8">
      <PageHeader
        ...
        subtitle={
          items.length === 0
            ? `Add your ${catalogueLabel.toLowerCase()} to build your website.`
            : `${items.length} item${items.length !== 1 ? 's' : ''} in your ${catalogueLabel.toLowerCase()}`
        }
        action={
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <SearchBar
          placeholder={`Search ${catalogueLabel.toLowerCase()}…`}
          value={searchQuery}
      ...
      <div className="bg-white rounded-xl border border-outline-variant shadow-sm overflow-hidden">
        ...
        <div className="hidden lg:grid grid-cols-[72px_1fr_160px_120px_150px_100px_120px] items-center px-4 sm:px-6 py-3 bg-surface-container-low border-b border-outline-variant text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">
          <div>Image</div>
          <div>Item</div>
          <div>Category</div>
          <div>Price</div>
          <div>Visible on Website</div>
          <div className="text-center">Highlight</div>
        ...
        {loading ? (
          ...
          <EmptyState
            icon={Package}
            title={searchQuery ? 'No results found' : `No ${catalogueLabel.toLowerCase()} yet`}
            description={
              searchQuery
                ? 'Try a different search term or remove the filter.'
                : `Start by adding your first ${itemNoun.toLowerCase()}.`
            }
            action={
              !searchQuery
                ? { label: `Add Your First ${itemNoun}`, onClick: handleOpenAdd }
                : undefined
            }
          />
        ) : (
      ...
      {items.length > 0 && !loading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-xl p-5 border border-outline-variant shadow-sm">
            ...
            <p className="text-xs text-on-surface-variant mt-0.5">Published to your website</p>
          </div>
          <div className="bg-white rounded-xl p-5 border border-outline-variant shadow-sm">
            <p className="text-[11px] font-medium text-on-surface-variant uppercase tracking-wider mb-1">
              Highlighted
            ...
            <p className="text-xs text-on-surface-variant mt-0.5">Shown on your homepage</p>
          </div>
          <div className="bg-white rounded-xl p-5 border border-outline-variant shadow-sm">
            <p className="text-[11px] font-medium text-on-surface-variant uppercase tracking-wider mb-1">
              Hidden
            ...
            <p className="text-xs text-on-surface-variant mt-0.5">Not visible on your website</p>
          </div>
        </div>
      )}
    ...
    >
      ...
      <div className="flex lg:block items-center gap-2 mb-2 lg:mb-0">
        ...
        <span className="lg:hidden text-xs text-on-surface-variant">
          {item.available ? 'Visible on Website' : 'Hidden'}
        </span>
      </div>
      ...
      <div className="hidden lg:flex justify-center">
        {toggling ? (
          ...
          <button
            ...
            title={item.featured ? 'Remove homepage highlight' : 'Highlight on Homepage'}
          >
            <Star
              size={18}
